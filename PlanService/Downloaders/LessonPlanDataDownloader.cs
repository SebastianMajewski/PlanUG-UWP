    ...
    public class LessonPlanDataDownloader : ILessonPlanDataDownloader
    {
        ...
        public async Task<List<Change>> DownloadChanges()
        {
            ...
            if (!string.IsNullOrEmpty(html))
            {
                ...
                return changesCollection?.Select(change => change.Descendants("td").ToList()).Select(tds => new Change { Group = tds[0].Element("nobr").InnerText, Lecturer = tds[1].Element("nobr").InnerText, Subject = tds[2].Element("nobr").InnerText, ClassesType = tds[3].Element("nobr").InnerText.NormalizeClassesType(), Changes = tds[4].Element("nobr").InnerText }).ToList();
            }
            else
            {
                throw new EntryPointNotFoundException();
